#При планировании межпланетной миссии в марсоход «зашили» ежедневные планы работ — списки задач на каждый день. 
# Но в реальности оказалось, что планы слишком объёмны: марсоход не успевает выполнить за день все запланированные задачи.
#В Центре управления марсоходами решили сократить ежедневный объём работ.
# Для этого марсоход ежедневно должен удалять из своего дневного расписания одну задачу. 
# Какую именно задачу удалить — определяют в ЦУМе. 
# Договориться толком руководители Центра не смогли и каждый день бросают жребий, 
# случайным образом выбирая задачу для удаления. Индекс удаляемого задания передаётся марсоходу.
...
# Для запуска тестов просто запустите скрипт через терминал.
# Например, так: python my_best_solution.py. Если не появилось никаких ошибок — всё в порядке, можно отправлять файл на проверку.
# Формат ввода
