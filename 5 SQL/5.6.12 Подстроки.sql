...
'номер стартовой позиции' — это начало выделяемой подстроки. Последний аргумент функции — длина подстроки, которую необходимо выделить, — необязательный. Если он не задан, функция извлечёт подстроку с начальной позиции до конца исходной строки:
