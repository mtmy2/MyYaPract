...
UPDATE dwh.craftsman_report_datamart SET
  ...
  8.
Сделайте INSERT FROM SELECT в таблицу dwh.load_dates_craftsman_report_datamart с временем последней загрузки данных в хранилище из блока dwh_delta. 
В блоке уже есть время последней загрузки (*_load_dttm), поэтому можно использовать максимальное значение из соответствующих столбцов для craftsman, customers, products. Если же *_load_dttm — NULL, используйте текущее время обновление витрины.
Чтобы получить значения по умолчанию, если колонка содержит значение NULL, используйте COALESCE:

-- вернёт 'default_value', если в columnA лежит NULL, или вернёт значение columnA, если там не NULL
SELECT COALESCE(columnA, 'default_value') FROM tableA; 
Чтобы получить текущие дату и время, нужно использовать NOW():

SELECT NOW(); 
Для поиска максимального значения, можно воспользоваться MAX():

-- вернёт максимальное значение в колонке columnA
SELECT MAX(columnA) FROM tableA; 
Для поиска самого большого значения из списка можно использовать GREATEST:

-- вернёт самое большое значение среди значений columnA, columnB, columnC
SELECT GREATEST(columnA, columnB, columnC) FROM tableA 
...
INSERT INTO dwh.load_dates_craftsman_report_datamart (
	...
	SELECT GREATEST(COALESCE(MAX(craftsman_load_dttm), NOW()), 
          ...
          COALESCE(MAX(products_load_dttm), NOW()))  -- напишите код здесь
	FROM dwh.dwh_delta;
-- проверка данных
-- SELECT * FROM dwh.load_dates_craftsman_report_datamart;




9.
Теперь все блоки готовы, и осталось их собрать в один CTE. Постройте итоговый CTE по блокам, которые вы создали на предыдущих шагах. Используйте код, который написали на предыдущих шагах для создания таблиц, только вместо таблиц будут блоки CTE.
