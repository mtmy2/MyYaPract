...
В прошлом спринте вы узнали, что сортировать записи можно по нескольким полям сразу. Так же и в оконных функциях: выражение с оператором ORDER BY может содержать сразу несколько полей. Порядок, в котором указаны поля, определяет приоритет ранжирования.
...
С помощью оконной функции ROW_NUMBER() и оператора ORDER BY можно выбрать запись с определённым рангом. Например, из таблицы online_store.sessions можно выбрать запись с рангом 100 в зависимости от даты сессии. Проранжированные записи можно поместить в общее табличное выражение, чтобы из него выбрать запись с нужным рангом. 
...
В прошлых уроках вы изучали важную особенность оконных функций — действия можно выполнять внутри заданного окна. Задать оператор окна можно с помощью PARTITION BY. Ранжировать данные также можно не по всей выборке, а по отдельным группам.
