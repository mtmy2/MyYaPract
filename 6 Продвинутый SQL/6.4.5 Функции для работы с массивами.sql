...
Нужен способ удобно и быстро вставлять данные из массива guid в таблицу drivers_schedule. В этой ситуации пригодится функция UNNEST. 
Она принимает в качестве аргумента массив и разворачивает его на набор строк. Например, так:
...
Итак, id сгенерируется автоматически. schedule_date — это текущая дата, поскольку данные вставляют один раз в день. 
depot_id — это целое число, id таксопарка. Оно одинаково для всего массива, потому что от каждого таксопарка приходит один массив. 
