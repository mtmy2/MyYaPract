...
FOR SHARE
NOWAIT; 
Этот запрос откроет транзакцию, выберет из таблицы products все товары с ценой более 2000, найдёт среди них второй по цене и заблокирует строку с ним в режиме SHARE. Это не позволит другим транзакциям менять поля для этой строки. При этом они смогут накладывать совместимые блокировки, в том числе в режиме SHARE. 
...
FROM students; 
Преподаватель открывает транзакцию и, выполнив SELECT, блокирует некоторые строки в  справочнике студентов в режиме NO KEY UPDATE, потому что поле со средним баллом не является ключом, ключ — student_id.
...
Если в запросе на выборку используют несколько таблиц, а заблокировать строки нужно только в некоторых из них, добавляют уточнение с ключевым словом OF, перечислив после него нужные таблицы или их псевдонимы, если они используются в запросе.
Например, тот же преподаватель после обновления среднего балла решил обновить данные по выплатам стипендий самым успешным студентам. Тогда строки в таблице со списком студентов ему блокировать не потребуется, поскольку они меняться не будут, а вот в таблице с выплатами будут:
