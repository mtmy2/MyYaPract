...
FOR NO KEY UPDATE	Разрешает блокирующей транзакции изменять только те поля, которые не входят в явно заданные первичные и внешние ключи — при таком изменении внешние ключи не меняются.	Исключительная	Одна транзакция
