...
В этом уроке вы познакомитесь с функциями смещения LEAD() и LAG(). 
Функции смещения возвращают данные из других записей в зависимости от их расстояния от текущего значения. 
Например, стоимость предыдущего заказа или следующий раз, когда пользователь входил на сайт. 
Эти функции помогают проанализировать изменения параметров по сравнению с предыдущими значениями. 
Например, можно для каждого пользователя сравнить стоимость текущего заказа со стоимостью предыдущего — 
это поможет оценить динамику затрат. Или можно рассчитать время, прошедшее после совершения предыдущего заказа или любого другого действия 
— это поможет проследить пользовательский путь. 
Функция LAG() позволяет возвращать предыдущие записи, а LEAD() — следующие. 
Например, выведем для каждой записи с заказом пользователя дату прошлого или следующего заказа 
— previous_order_dt и next_order_dt соответственно.
...
А значение по умолчанию указывает, какое значение нужно вернуть в случае, если нужной строки в таблице нет. 
В запросе выше функциям LEAD() и LAG() передали всего один аргумент — поле event_dt. 
Дело в том, что у двух других аргументов есть значения по умолчанию. 
Если не указать аргумент для смещения, он будет равен 1. 
В таком случае функции будут возвращать записи, которые находятся либо сразу за текущей записью, либо перед ней. 
...
ORDER BY в определении окна задаёт последовательность, 
по которой СУБД будет искать следующие или предыдущие значения. 
Если его не указывать, ошибки не будет, но значения будут выбраны в той последовательности, 
в которой они записаны в БД, поэтому результат может отличаться от ожидаемого.
Рассмотрим действие функций смещения подробней. 
Напомним, что функция LEAD() возвращает следующие записи относительно текущей.
Аргументы 1 и NULL можно не указывать, и функции LEAD(event_dt, 1, NULL) и LEAD(event_dt) дадут одинаковые результаты. 
Если подставить в предыдущий код эти аргументы, то получим то же самое:
